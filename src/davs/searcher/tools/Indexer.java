...
public class Indexer
	{
		...
		private String dataDir; //path to data to be indexed, i.e. what should I put in index
		
		
		/**
		 * Create new instance of class responsible for index creation. 
		 * 
		 * @param indexDir Folder where index will be stored
		 * @throws IOException 
		 */
		public Indexer(String indexDir) throws IOException
			{
				...
				setAnalyzer(new StopAnalyzer(currentVersion)); // name1a.name2b_name3... => (to tokens) namea, nameb, name...
		 ...
		 * 
		 * @param isAppendingIndex If true append to existing index, otherwise create new index
		 */
		private IndexWriter getIndexWriter(boolean isAppendingIndex) 
			{
				...
				IndexWriterConfig config = new IndexWriterConfig(currentVersion, getAnalyzer());	
				
				if(isAppendingIndex)
					config.setOpenMode(IndexWriterConfig.OpenMode.APPEND);
				else config.setOpenMode(IndexWriterConfig.OpenMode.CREATE);
		...
		/**
		 *	//TODO look into tracking hash checksums of each file to update specific file 
		 *
		 * Perform actual indexing. 
		 * 
		 * @param isAppendingIndex If true append to index, otherwise create new index
		 * @param indexFullContents If true save full text in index
		 ...
		 */
		public int indexDir(String dataDir, boolean isAppendingIndex, boolean indexFullContents)
			{
								...
								if(indexFullContents)
									{
										...
										document.add( new Field(IndexField.fulltext , content) );
									}
								
								//e.g. x.y will be searchable as x and y
								document.add( new Field(IndexField.filename, file2Index.getName(), Field.Store.YES, Field.Index.ANALYZED_NO_NORMS ) );
								document.add( new Field(IndexField.filepath, file2Index.getCanonicalPath(), Field.Store.YES, Field.Index.ANALYZED_NO_NORMS) );
				...
				while(!holdAllFiles.isEmpty())
					{
						...
						else if(!file.isHidden() && file.canRead() && file.exists() && isAcceptedFileFormat(file, new Tika()))
		 ...
		 * 
		 * @param file Detect format on this file
		 * @param detectFileType Tika facade that performs file detection
