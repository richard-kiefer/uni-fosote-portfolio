...
public class SpellCheck
	{
		public static int numberOfSuggestions = 3; //Display only first 3 spell suggestions
		...
		public SpellCheck() throws IOException
			{
				...
				getSpellCheck().indexDictionary(new PlainTextDictionary(new File("./dict")));//dict is dictionary file
		...
		public String[] getSpellSuggestions(String word, int numOfSuggestions)
			{
				...
				try
					{
						//If user enters multiple words in a string(e.g. "cat dog"), it is necessary to break it
						//into multiple string and get suggestions for each word(e.g. "cat", "dog")
						String[] potentialSuggestions = splitString(word);
						
						if(potentialSuggestions == null || !(potentialSuggestions.length > 0) )
							return null;
						...
						//Cycle through each split word (e.g. cycle through cat first, then through dog...)
						for(int i=0; i<potentialSuggestions.length; i++)
							{
								//Get suggestion for each split word
								String[] tmp = getSpellCheck().suggestSimilar(potentialSuggestions[i], numberOfSuggestions);
								...
								if (tmp != null && tmp.length > 0)
									{
										...
										for(int j=0; (j<tmp.length && j<results.length); j++)
											{
												tmp[j] = tmp[j].replaceAll("^\\s+", ""); //Remove leading whitespace
